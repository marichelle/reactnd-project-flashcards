// standard
export const black = '#000';
export const white = '#fff';

// custom [https://coolors.co/131200-407f3e-89b449-dbd468-2f7cf7-985556-e68a8c-e7e0c4-f0f0f0]
const blackChocolate = '#131200';
const chineseViolet = '#714E76';
const budGreen = '#89b449';
const azure = '#2f7cf7';
const lightCoral = '#e68a8c';
const cultured = '#f0f0f0';

// color scheme
export const primary = chineseViolet;
export const secondary = blackChocolate;
export const tertiary = white;
export const success = budGreen;
export const danger = lightCoral;
export const link = azure;
export const muted = cultured;

// other styles
export const borderRadius = 5;
export const buttonFontSize = 16;
export const buttonFontWeight = 'bold';
